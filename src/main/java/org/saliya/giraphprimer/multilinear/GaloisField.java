...
public class GaloisField implements Serializable{
    ...
    private GaloisField(int fieldSize, int primitivePolynomial) {
        if (fieldSize <= 0) {
            throw new IllegalArgumentException("fieldSize must be a positive integer");
        }
        if (primitivePolynomial <= 0) {
            throw new IllegalArgumentException("primitivePolynomial must be a positive integer");
        }
    ...
    public int add(int x, int y) {
        if ((x < 0 || x >= getFieldSize()) || (y < 0 || y >= getFieldSize())) {
            throw new IllegalArgumentException("x and y must be in the range [0, fieldSize)");
        }
        return x ^ y;
    }
    ...
    public int ffMultiply(int x, int y) {
    ...
    public int power(int x, int n) {
        if ((x < 0 || x >= getFieldSize())) {
            throw new IllegalArgumentException("x must be in the range [0, fieldSize)");
        }
        if (n == 0) {
            return 1;
        }
    ...
    public void solveVandermondeSystem(int[] x, int[] y, int len) {
        ...
        if (x.length > len || y.length > len) {
            throw new IllegalArgumentException("x and y must be less than or equal to len");
        }
